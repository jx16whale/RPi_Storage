        ...
        while True:
            try:
                if self.clientsocket == None:
                    ...
                    print(f"[AND_SERVER] Connection from {str(address)} has been established on RFCOMM channel 1!")
                    break
...
if __name__ == '__main__':
    ...
    try:
        while True:
            A.recv()
            A.send(input(f"[AND_SERVER] Send Message: "))
    except KeyboardInterrupt:
        android.disconnect_server()
        print("[AND_SERVER] Terminating the program now...")